function varargout = ctm4xas523(varargin)
...
function calc_spec_Callback(hObject, eventdata, handles)
  ...
  if handles.RIXS, setvalTag('autoplot',0); end
...
function handles_out=update_config(handles)
    ...
    if getvalTag('symmetry')>3,handles.D3d=1; else handles.D3d=0; end
...
function [file_created,quadTr,xes,bindEs]=make_plot_files(handles,file2plot)
    ...
    if fid_test==-1
        ...
        elseif F4==1,
            spectra=1;
            bindEs=num2str(str2num(bindEs)-100);
            if ChBxStatus('lorenz_split')
            ...
            for jj_2=1:length( batch_param2_mesh)
                for jj_3=1:length( batch_param3_mesh)
                    ...
                    for ii=1:length(tag_param1)
                        ...
                        fn_ext1=strcat({' '},batch_param1,{' '},num2str(batch_param1_mesh(jj_1),'%5.3f'));
                    end
                    if num_batch_param>1
                        for ii=1:length(tag_param2)
                            setvTag(tag_param2{ii},batch_param2_mesh(jj_2))
                        end
                        fn_ext2=strcat({' '},batch_param2,{' '},num2str(batch_param2_mesh(jj_2),'%5.3f'));
                    else
                        fn_ext2='';
                    end
                    if num_batch_param>2
                        ...
                        fn_ext3=strcat({' '},batch_param3,{' '},num2str(batch_param3_mesh(jj_3),'%5.3f'));
                    else
                        fn_ext3='';
                    end
...
function fit_Callback(hObject, eventdata, handles)
    ...
    if ~(isempty(exp2fit)&&isempty(sims2fit))
        ...
        if ~isempty(sims2fit)
            ...
            fit_plot=plot(h1a,x_spl,flip*YY_spl_sum_norm*max_exp,'r','LineWidth',0.5);
            set(fit_plot,'Tag','fit');
