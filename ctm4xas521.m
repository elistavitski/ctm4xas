function varargout = ctm4xas521(varargin)
                ...
                while oba_str~=-1
                    ...
                    if oba_str~=-1
                        if strcmp(oba_str(2:7), 'Ground')
                            ...
                            oba_str=fgets(fid_oba);
                            i1=strfind(oba_str,'0.');
                            w1(gn_i)=str2num(oba_str(i1(1):i1(2)-1));
                            w2(gn_i)=str2num(oba_str(i1(2):i1(3)-1));
        ...
        for jj=1:length(list_files2plot);
            ...
            if fid~=-1,
                ...
                if created
                    ...
                    plot_spectrum(list_files2plot{jj},jj,quadTr,xes,bindE,h1a,indx_stack);
                    hold(h1a,'on');
                end
            end
            fclose all
...
% Plot spectra
function plot_spectrum(file2plot,indx,quadTr,xes,bindE,h1a,jj)  
    no_plot=0;
    color_arr='gbrm'; 
    ...
    if (quadTr==0)
        ...
        if isempty(s0_sticks)&isempty(s2_sticks)&...
            ...
            %spec_to_plot(:,2)= s0_2plot;
        end
    end
    if ~no_plot
        if ~ChBxStatus('nonorm')
            spec_to_plot(:,2)=normalize(spec_to_plot(:,2));
            sticks_to_plot(:,2)=normalize(sticks_to_plot(:,2));
        end
        bindE=str2num(bindE);
        if xes
